...
export const initPagination = ({pages, fromRow, toRow, totalRows}, createPage) => {
    ...
    const applyPagination = (query, state, action) => {
        ...
        if (action) switch(action.name) {
            ...
            case 'next': page = Math.min(pageCount || Infinity, page + 1); break;    // переход на следующую страницу
    ...
    const updateNavigationButtons = () => {
        ...
        if (navButtons) {
            navButtons.forEach(button => {
                const isDisabled = (button.name === 'prev' || button.name === 'first') ? currentPage === 1 :
                                   (button.name === 'next' || button.name === 'last') ? currentPage === pageCount :
                                   false;
